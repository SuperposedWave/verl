...
class MultiImageZoomInTool(BaseTool):
    """A tool for zooming in on an selected image among multi images by cropping it based on a bounding box.
                ...
                "parameters": {
                    ...
                    "properties": {
                        "index": {
                            "type": "integer",
                            "description": (
                                "The index of the image to zoom in on, starting from 1."
                            ), 
                        },
                        "bbox_2d": {
                            "type": "array",
    ...
    async def create(self, instance_id: Optional[str] = None, **kwargs) -> tuple[str, ToolResponse]:
        ...
        Args:
            ...
            **kwargs: Should contain 'image' key with image data, or 'create_kwargs'
                containing {'image': image_data}. Image can be a list of images or
                a single image in any of the following formats:
        ...
        lst_images = [fetch_image({"image": image}) for image in images]
        logger.warning(f"Fetched {len(lst_images)} images for instance {instance_id}")
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        if not index:
            return ToolResponse(text="错误：缺少index参数。"), -0.05, {"success": False}

        if not bbox_2d or len(bbox_2d) != 4:
            return (
                ToolResponse(text="错误：缺少bbox_2d参数，或该参数不是由4个数字组成的列表。"),
                -0.05,
                {"success": False},
            )

        instance_data = self._instance_dict[instance_id]
        image = instance_data["image"][index - 1] if 0 < index <= len(instance_data["image"]) else None
        if image is None:
            return ToolResponse(text=f"错误：在index{index}处未找到图像。"), -0.05, {"success": False}
        ...
        try:
            ...
            if resized_bbox is None:
                error_msg = (
                    f"错误：指定的边界框 {bbox_2d} 无效，"
                    f"或裁剪结果小于最小尺寸 {self.MIN_DIMENSION}x{self.MIN_DIMENSION}。"
                )
            ...
            return ToolResponse(text=f"处理图像放大时出错：{e}"), -0.05, {"success": False}

        response_text = f"成功将图像放大到区域{bbox_2d}。"
        if label:
            response_text = f"成功将图像放大到标签为{label}的区域{bbox_2d}。"

...
#     """
#     print(f"[DEBUGGGGGG-ZX] Predict String: {predict_str}")
#     # format_score
#     is_format_error = False
...
#     res = 0.8 * acc_reward + 0.2 * format_reward + 1.2 * tool_reward
...
def compute_score(predict_str: str, ground_truth: str, extra_info=None) -> float:
    ...
    # format_score
    is_format_error, error_messages = check_model_format(predict_str)
    count_vision = predict_str.count("<|vision_start|><|image_pad|>")
    if count_vision > 0:
        is_call_tool = True
    else:
        is_call_tool = False
